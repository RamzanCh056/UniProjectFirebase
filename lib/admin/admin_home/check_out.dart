...
class _MyDemoPageState extends State<MyDemoPage> {
  ...
  Future<void> _cashPayClicked(BuildContext context) async {
    ScaffoldMessenger.of(context)
        .showSnackBar(const SnackBar(content: Text('Payments not available on Gilgit we are working on it')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final List<PriceItem> _priceItems = [
      PriceItem(name: 'Product', quantity: 1, totalPriceCents: 5200),
...
class DemoOnlyStuff {
  ...
  Future<void> callTransactionApi(
    ...
    await Future.delayed(const Duration(seconds: 2), () {
      if (shouldSucceed) {
        Fluttertoast.showToast(msg: 'Payments not available on Gilgit we are working on it');
